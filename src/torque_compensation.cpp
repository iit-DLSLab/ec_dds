...
TorqueCompensation::TorqueCompensation(const std::vector<double>& data)
    : torque_comp_vars(4) // temp,w,tau_des
{
    init(data);
}
...
void TorqueCompensation::init(const std::vector<double>& data){
    // -- table for torque compensation
    for(int j=0; j<data.size()/torque_comp_vars;j++){ //cicling over rows
        // getting temp,w,tau_des
        std::vector<double> key;
        for (int k=0; k<torque_comp_vars-1;k++){
            key.push_back(data[j*torque_comp_vars+k]); 
        }
        torque_comp_table[key] = data[j*torque_comp_vars+torque_comp_vars-1]; // alpha
        temp_keys.push_back(key[0]);
        w_keys.push_back(key[1]);
        tau_des_keys.push_back(key[2]);
    }

    sortAndUniqueVec(temp_keys);
    sortAndUniqueVec(w_keys);
    sortAndUniqueVec(tau_des_keys);
}
...
void TorqueCompensation::sortAndUniqueVec(std::vector<double>& vec){
    ...
    // std::cout << "Sorted and unique values: ";
