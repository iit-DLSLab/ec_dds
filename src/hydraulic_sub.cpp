...
void hydraulic_sub::SubListener::on_subscription_matched(
        DataReader*,
...
void hydraulic_sub::SubListener::on_data_available(
        DataReader* reader)
{
    SampleInfo info;
    if (reader->take_next_sample(&valve, &info) == ReturnCode_t::RETCODE_OK)
    {
        if (info.instance_state == ALIVE_INSTANCE_STATE)
        {
            ...
            std::cout << "Index: " << valve.index() << " ID:" << valve.id() << " Encoder Pos: " << valve.encoderPos()  << " Pressure1: " << valve.pressure1() << \
                " Pressure2: " << valve.pressure2() << " Temp: " << valve.temp() << " Torque: " << valve.torque() << std::endl;
        }
    }
}

void hydraulic_sub::run()
{
